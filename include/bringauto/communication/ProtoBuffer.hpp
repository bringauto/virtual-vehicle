...
namespace bringauto::communication {
	...
	public:
		ProtoBuffer(const std::shared_ptr<bringauto::virtual_vehicle::GlobalContext> &globalContext,
					uint32_t statusPeriodMs,
					const std::string &ipAddress, int port,
					const std::shared_ptr<bringauto::virtual_vehicle::GlobalContext> &context) : ICommunication(
				globalContext, statusPeriodMs),
		 ...
		 */
		void updateStatus(double lat, double lon, double speed, State state, const std::string &nextStop) override;
	...
	private:
		...
		std::string
		generateCarStatusString(double lat, double lon, double speed, State state, const std::string &nextStop);
